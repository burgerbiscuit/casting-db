...
export async function POST(req: NextRequest) {
  ...
  if (body.role && String(body.role).length > 100) return NextResponse.json({ error: "Input too long" }, { status: 400 })
  if (body.company && String(body.company).length > 200) return NextResponse.json({ error: "Input too long" }, { status: 400 })
